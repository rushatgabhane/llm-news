...
async def fetch_hackernews_top_stories(logger, limit=25):
    try:
        async with httpx.AsyncClient() as client:
            ...
            tasks = [
                client.get(HACKERNEWS_ITEM_URL.format(story_id))
            ...
            logger.info(
                f"[Hacker News API] Fetched metadata for {len(metadata)} articles."
            )
            return metadata
